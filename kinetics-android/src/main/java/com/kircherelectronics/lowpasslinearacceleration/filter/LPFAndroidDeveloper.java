 ...
 * @author Kaleb
 * @version %I%, %G%
 * @see <a>http://developer.android.com/reference/android/hardware/SensorEvent.html</a>
 */
public class LPFAndroidDeveloper implements LowPassFilter {
    ...
    private static final float timeConstant = 0.18f;
    private float alpha = 0.9f;
    private final float timestampOld = System.nanoTime();
    private int count = 0;
    // Gravity and linear accelerations components for the
    // Wikipedia low-pass filter
    private final float[] gravity = new float[]
            {0, 0, 0};
    private final float[] linearAcceleration = new float[]
            {0, 0, 0};
    // Raw accelerometer data
    private final float[] input = new float[]
            {0, 0, 0};
